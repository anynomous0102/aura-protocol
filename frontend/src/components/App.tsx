...
const callAI = async (
  ...
  const nodeAddress = typeof model !== 'string' ? model.nodeAddress : undefined;
  const nodeProvider = typeof model !== 'string' ? model.provider : undefined;
  const effectiveUserEmail = userEmail || "anonymous";

  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort("AURA request timed out after 180 seconds."), 180000);
    ...
    const errorMessage =
      error?.name === "AbortError" || String(error?.message || "").toLowerCase().includes("abort")
        ? "AURA request timed out before this model returned. Try fewer connected models or a faster/free chat model."
        : error?.message || "Unknown connection error";
    console.warn(`Connection failed for ${modelId}:`, errorMessage);
    const lastUserMessage = messages.filter(m => m.role === 'user').pop()?.text || "";

    if (overrideSystem && overrideSystem.includes("prompt engineer")) {
      return `✨ Enhanced: ${lastUserMessage} (Please ensure high detail and clear formatting).`;
    }
...
const INITIAL_MODELS: Model[] = [
  ...
  { id: "claude-sonnet-4-6", name: "Claude Sonnet 4.6", provider: "Anthropic (Persona)", hex: "#D97706", tw: "bg-amber-600" },
